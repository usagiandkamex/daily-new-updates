...
(3〜5個。**AIOps**（AIを活用したIT運用自動化・異常検知・予測分析）を重点的に取り上げること。Microsoft Azure Monitor・System Center等のMicrosoft製品によるAIOpsも優先的に含める。ITSM・DevOps・エンドポイント管理・MSP・オブザーバビリティなど、IT運用全般のトレンドも含める。各トピックは見出し・要約・影響・参考リンクで構成)
...
def generate_article(
    ...
    news_lists = [azure_news, tech_news, business_news, sns_news, itops_news]
